<template>
  <div class="list_exam">
    ...
    <div v-if="list_content_exame" class="list_exam_child">
      <!-- filter  -->
      <div class="parent_filter">
        ...
        <div>
          <strong>số lượng {{ list_content_exame.length }}</strong>
        </div>
      </div>
      <!-- end filter -->
      <!-- List exam -->
      <div class="list_exams">
        <table v-if="list_content_exame.length > 0" class="table">
          <tr>
            <th>Id</th>
            <th>Tên nội dung</th>
            <th>Chức năng</th>
          ...
          <tr
            :id="'item' + item.id"
            v-for="item in list_content_exame"
            :key="item.id"
          >
            <td>{{ item.id }}</td>
            <td>{{ item.name }}</td>
...
export default {
  ...
  updated() {
    ...
    for (var item in this.list_content_exame) {
      const div = document.getElementById(
        "item" + this.list_content_exame[item].id
      );
  ...
  setup() {
    ...
    const list_content_exame = computed(() => {
      if (key_search.value != null && key_search.value != "") {
        return store.state.content_exame.content_exame.filter(
          (e) =>
    ...
    const deletes = (id) => {
      ...
      store.dispatch("content_exame/delete", id);
    };
    ...
    const run_app = watch(this_page, () => {
      ...
      for (var item in list_content_exame.value) {
        const div = document.getElementById(
          "item" + list_content_exame.value[item].id
        );
        if (item >= page_start && item <= page_end) {
          div.classList.contains("hide") ? div.classList.remove("hide") : null;
          div.classList.add("show");
    ...
    const count_page = computed(() => {
      if (list_content_exame.value == null) {
        return 0;
      }
      return Math.ceil(list_content_exame.value.length / limit_page.value);
    });
