<template>
  <div class="add_exam">
    <h1>
      {{ this.$route.query.action == "edit" ? "Cập nhật" : "Thêm" }} nội dung
      thi
    </h1>
    <form @submit.prevent="submit" class="from_add_exam">
      <div class="group_add_exam">
        <div class="name_add_exame">
...
export default {
  ...
  created() {
    const check = setInterval(() => {
      if (store.state.content_exame.content_exame != null) {
        clearInterval(check);
        if (this.$route.query.action) {
          this.$route.query.action == "edit"
            ? ((store.state.content_exame.data_post.id = Number(
                this.$route.query.id
              )),
              (store.state.content_exame.data_post.name = store.getters[
                "content_exame/getbyid"
  ...
  setup() {
    ...
    const data = computed(() => {
      return store.state.content_exame.data_post;
    });
    const err = computed(() => {
      return store.state.content_exame.err;
    });
    const submit = () => {
      !data.value.name
      ...
      !err.value.name
        ? data.value.id == null
          ? (store.dispatch("content_exame/create"),
            (data.value.name = null),
            toast.success("Tạo thành công"))
          : (store.dispatch("content_exame/update"),
            (store.state.content_exame.data_post.id = null),
