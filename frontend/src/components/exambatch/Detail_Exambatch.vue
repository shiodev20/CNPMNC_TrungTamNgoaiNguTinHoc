    ...
    <div class="Upperbar">
      <div class="upperbar_add">
        ...
        <div class="upperbar_item">
          ...
          <input v-model="exam_id.name" disabled type="text" />
        </div>
      </div>
    </div>
      ...
      <div class="list_exams">
        <div class="parent_customer">
          <div v-if="cus" class="filter_customer">
            <label class="lb_filter_cus">
              <input
                v-model="$store.state.cus_exam.status_chose"
                type="radio"
                :value="1"
                name="dd"
              />
              <span
                >Thuộc đợt thi <strong>{{ count_cuspay }}</strong></span
              >
            </label>
            <label class="lb_filter_cus">
              <input
          ...
          <div class="parent_chonse">
            <div class="chonse">
              <strong>Số lượng {{ list_customer.length }}</strong>
            </div>
            <!-- <div v-if="$store.state.cus_exam.status_chose == 3" class="chonse">
        ...
        <table v-if="list_customer.length > 0" class="table">
          <tr>
            ...
            <th>Tên</th>
            <th>Ngày sinh</th>
            <th v-if="$store.state.cus_exam.status_chose != 1">
              Hạn thanh toán
            </th>
            <th v-if="$store.state.cus_exam.status_chose != 1">Chức năng</th>
          </tr>
          <tr
            :id="'item' + item.id"
          ...
          >
            ...
            <td
              v-if="$store.state.cus_exam.status_chose != 1"
              :id="'clock' + item.id"
            >
            ...
            <td v-if="$store.state.cus_exam.status_chose != 1" class="setting">
              <button
                v-if="$store.state.cus_exam.status_chose == 3"
              ...
              <button
                ...
                v-if="
                  ...
                  $store.state.cus_exam.status_chose != 3
                "
              >
...
export default {
  ...
  setup() {
    ...
    const data_id = computed(() => {
      return store.getters["exambatch/getbyid"](data.value.id)[0];
    });

    const exam_id = computed(() => {
      return store.getters["exam/getbyid"](data_id.value.exam_id);
    });
    ...
    const list_customer = computed(() => {
      if (avaible.value != false) {
        array.value.forEach((e) => {
          store.dispatch("cus_exam/delete", e);
        });
        store.dispatch("cus_exam/get_all");
        // eslint-disable-next-line vue/no-side-effects-in-computed-properties
        array.value = [];
        console.log("oweiofjewijfeiwj");
        // eslint-disable-next-line vue/no-side-effects-in-computed-properties
      ...
      if (store.state.cus_exam.status_chose == 2) {
        return store.getters["cus_exam/getbyidexam_payfalse"](
          store.state.exambatch.data_post.id
        );
      }

      if (store.state.cus_exam.status_chose == 3) {
        return store.getters["cus_exam/getbyidexam_notpay"](
          store.state.exambatch.data_post.id
        );
      }
      return store.getters["cus_exam/getbyidexam"](
        store.state.exambatch.data_post.id
      );
    });
    const onsubmit = () => {
    ...
    const deleteid = (id) => {
      ...
      store.dispatch("cus_exam/deleteid", id);
    };
    ...
    const add_cus_class = (value) => {
      ...
      const datapost = reactive({
        ...
        price: store.getters["exam/getbyid"](
          store.getters["exambatch/getbyid"](
            store.state.exambatch.data_post.id
          )[0].exam_id
        ).price,
        id_exambactch: store.state.exambatch.data_post.id,
        id_cus: value,
      });
      ...
      store.dispatch("cus_exam/updatesx", data);
      toast.success("Thêm vào thành công");
      //add bill class
      store.dispatch("bill_cus_exam/add_cus", datapost);
    };
    return {
      add_cus_class,
      delete_all,
